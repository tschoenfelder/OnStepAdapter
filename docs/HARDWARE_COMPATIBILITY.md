...
OnStepAdapter `0.3.0` was physically tested with:
...
- bounded RA/DEC center and guide corrections with directional stop;
- independent RA `+1 h/-1 h` and DEC `+10/-10 degrees` coordinate movement;
- recovery of the initial coordinate target after the four-leg path;
...
- final HOME-to-PARK confirmation after every `0.3.0` hardware motion test.
